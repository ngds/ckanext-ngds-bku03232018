...
def datastore_spatialize(context, data_dict):
    ...
    **Results:**
    ...
    print data_dict['connection_url']
...
def datastore_is_exposed_as_layer(context, data_dict):
    ...
    **Results:**
    ...
    resource = toolkit.get_action("resource_show")(context, {"id": res_id})
    layer_name = resource.get("geoserver_layer_name")
    if layer_name:
        geoserver = Geoserver.create_from_ckan_config()

        # Verifies if the user calling the method has permission to execute this call
        p.toolkit.check_access('datastore_is_exposed_as_layer', context, data_dict)

        # Look for the layer
        if geoserver.get_resource(layer_name):
            return {"is_exposed_as_layer": True}

    return {"is_exposed_as_layer": False}
